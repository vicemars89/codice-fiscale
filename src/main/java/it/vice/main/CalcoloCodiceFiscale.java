...
public class CalcoloCodiceFiscale {
	...
	public static String calcoloCognome(String cognomePersona) {
		...
		if (arrayCognome.length > 3) {
			for (String lettera : arrayCognome) {
				if (toReturn.length() < 3 && !lettera.equalsIgnoreCase("a") && !lettera.equalsIgnoreCase("e")
						&& !lettera.equalsIgnoreCase("i") && !lettera.equalsIgnoreCase("o")
	...
	public static String calcoloNome(String nomePersona) {
		...
		String[] vocali = calcoloVocali(arrayNome).split("");
		String toReturn = "";
		int count = 1;
		if (consonanti.length >= 4) {
			if (arrayNome.length > 3) {
				for (String lettera : consonanti) {
					if (toReturn.length() < 3 && (count == 1 || count == 3 || count == 4)
							&& !lettera.equalsIgnoreCase("a") && !lettera.equalsIgnoreCase("e")
							&& !lettera.equalsIgnoreCase("i") && !lettera.equalsIgnoreCase("o")
							&& !lettera.equalsIgnoreCase("u")) {
						toReturn = toReturn.concat(lettera);
					}
			...
			for (String lettera : consonanti) {
				if (toReturn.length() < 2 && !lettera.equalsIgnoreCase("a") && !lettera.equalsIgnoreCase("e")
						&& !lettera.equalsIgnoreCase("i") && !lettera.equalsIgnoreCase("o")
						&& !lettera.equalsIgnoreCase("u")) {
					toReturn = toReturn.concat(lettera);
	...
	public static String calcoloConsonanti(String[] nome) {
		...
		for (String lettera : nome) {
			if (!lettera.equalsIgnoreCase("a") && !lettera.equalsIgnoreCase("e") && !lettera.equalsIgnoreCase("i")
					&& !lettera.equalsIgnoreCase("o") && !lettera.equalsIgnoreCase("u")) {
	...
	public static String calcoloVocali(String[] nome) {

		logger.debug("calcoloVocali()...");
		String vocaliNome = "";
		for (String lettera : nome) {
			if (lettera.equalsIgnoreCase("a") || lettera.equalsIgnoreCase("e") || lettera.equalsIgnoreCase("i")
					|| lettera.equalsIgnoreCase("o") || lettera.equalsIgnoreCase("u")) {
				vocaliNome = vocaliNome.concat(lettera);
			}
		}
		return vocaliNome;
	}
